...
- **Phase 9.5 (Abstraction des Commandes RHI) :** Création de l'interface `IRenderCommandList` (et son implémentation `VulkanCommandList`). Migration de toute la logique de rendu (`vk_engine_frame.cpp`, `vk_engine_ibl.cpp`) pour utiliser cette liste de commandes au lieu de méthodes proxy sur l'objet `RHI`. Nettoyage des anciennes méthodes `Cmd*` dans `IRHI` et refactoring de la complexité cognitive de `main.cpp`.
