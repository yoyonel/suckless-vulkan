...
def main():
    ...
    for f in files_to_compare:
        ...
        if not ok:
            ...
            status = "PASS" if mse < mse_threshold and ssim_val >= ssim_threshold else "FAIL"
            if status == "FAIL": all_passed = False
            print(f"{f:<25} | {mse:<12.6f} | {ssim_val:<10.4f} | {status} (Max: {max1:.2f} vs {max2:.2f})")
